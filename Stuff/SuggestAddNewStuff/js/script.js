...
$(document).ready(function(){
	$(document).click(function(){
		$("#ajax_response_addnewstuff").fadeOut('slow');
	});
	$("#txtStuffname").focus();
	var offset = $("#txtStuffname").offset();
	var width = $("#txtStuffname").width()-2;
	$("#ajax_response_addnewstuff").css("left",offset.left);
	$("#ajax_response_addnewstuff").css("width",width);
	$("#txtStuffname").keyup(function(event){
		 ...
		 if(txtStuffname.length)
		 {
			 if(event.keyCode != 40 && event.keyCode != 38 && event.keyCode != 13)
			 {
				 ...
				 $.ajax({
				   type: "POST",
				   url: "suggestaddnewstuff/ajax_server.php",
				   data: "data="+txtStuffname,
				   success: function(msg){	
					if(msg != 0)
					  $("#ajax_response_addnewstuff").fadeIn("slow").html(msg);
					else
					{
					  $("#ajax_response_addnewstuff").fadeIn("slow");
					  $("#ajax_response_addnewstuff").html('<div style="text-align:right;">No Matches Found</div>');
					}
	...
	$("#ajax_response_addnewstuff").mouseover(function(){
