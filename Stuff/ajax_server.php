...
$objConnection->Open();
$keyword = $_POST['data'];
$sql = "select Title from tblCategory where Title Like '".  addslashes($keyword)."%' or upper(convert (Title using latin1)) like '" . addslashes(strtoupper($keyword)) . "%' limit 0,8";
//$sql = "select name from ".$db_table."";
$result = mysql_query($sql) or die(mysql_error());
if (mysql_num_rows($result))
{
    ...
    while ($row = mysql_fetch_array($result))
    {
        ...
        $final = '<span class="bold">' . $first . '</span>' . $last;
        //echo '<li><a href=\'javascript:void(0);\'>' . $final . '</a></li>';
        echo '<li><a href="#" style="text-align:right">' . $final . '</a></li>';
    }
    echo "</ul>";
}
else
