...
$(document).ready(function(){
	...
	$("#ajax_response_search").css("left",offset.left);
	$("#ajax_response_search").css("width",width);
	$("#txtsearch").keyup(function(event){
		 ...
		 if(txtsearch.length)
		 {
			 if(event.keyCode != 40 && event.keyCode != 38 && event.keyCode != 13)
			 {
				 ...
				 $.ajax({
				   type: "POST",
				   url: "SuggestSearch/ajax_server.php",
				   data: "data="+txtsearch,
				   success: function(msg){	
					if(msg != 0)
					  $("#ajax_response_search").fadeIn("slow").html(msg);
					else
					{
					  $("#ajax_response_search").fadeIn("slow");
					  $("#ajax_response_search").html('<div style="text-align:right;display:none;visibility:hidden;"></div>');
					}
	...
	$("#ajax_response_search").mouseover(function(){
