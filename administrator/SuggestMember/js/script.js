...
$(document).ready(function(){
	$(document).click(function(){
		$("#ajax_response_member").fadeOut('slow');
	});
	$("#txtsearch").focus();
	var offset = $("#txtsearch").offset();
	var width = $("#txtsearch").width()-2;
	$("#ajax_response_member").css("left",offset.left);
	$("#ajax_response_member").css("width",width);
	$("#txtsearch").keyup(function(event){
		 ...
		 if(txtsearch.length)
		 {
			 if(event.keyCode != 40 && event.keyCode != 38 && event.keyCode != 13)
			 {
				 ...
				 $.ajax({
				   type: "POST",
				   url: "suggestmember/AutoSuggestMember.php",
				   data: "data="+txtsearch,
				   success: function(msg){	
					if(msg != 0)
					  $("#ajax_response_member").fadeIn("slow").html(msg);
					else
					{
					  $("#ajax_response_member").fadeIn("slow");
					  $("#ajax_response_member").html('<div style="text-align:right;display:none;visibility:hidden;"></div>');
					}
			 ...
			 else
			 {
				switch (event.keyCode)
				{
				 ...
				 break;
				 case 13:
					$("#ajax_response_search").fadeOut("slow");
	...
	$("#ajax_response_member").mouseover(function(){
