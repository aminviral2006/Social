...
if(isset($_REQUEST['txtsearch']) && $_REQUEST['txtsearch']!="")
{
    ...
    //Pagination
    $_SESSION['currentpagelimit']=$_SESSION['TotalRecordsOnAdminMemberSearchPage'];
    $pages = new Paginator();
    $num_rows=$objPlMemberRegistration->PlTotalSearchedResult($_REQUEST['txtsearch']);
    $style="
        <!--Paging-->
            <style type='text/css'>
            .paginate {
    ...
    $limit=$pages->limit;
    $output=$objPlMemberRegistration->PlGetMemberSearchedListOnAdminPage($_REQUEST['txtsearch'],$limit);
    ...
    else
    {
        echo "<div align='center'>
            <h4 style='color:red'>The member name <span style='color:blue;'>'".$_REQUEST['txtsearch']."'</span> is not found.</h4></div>";
    }
    //Pagination
...
{
    header("location:SearchMember.php?msg=Search Name cannot be blank");
}
