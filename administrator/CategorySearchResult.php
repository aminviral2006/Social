...
if(isset($_REQUEST['txtsearch']) && $_REQUEST['txtsearch']!="")
{
    ...
    $pages = new Paginator();
    $num_rows=$objPlCategory->PlTotalCategoriesSearchedResultOnAdminPage($_REQUEST['txtsearch']);
    $style="
        <!--Paging-->
            <style type='text/css'>
            .paginate {
    ...
    $limit=$pages->limit;
    $output=$objPlCategory->PlGetSearchedCategoriesOnAdminPage($limit,$_REQUEST['txtsearch']);
    ...
    else
    {
        echo "<div align='center'>
            <h4 style='color:red'>The Category Title <span style='color:blue;'>'".$_REQUEST['txtsearch']."'</span> is not found.</h4></div>";
    }
    //Pagination
...
{
    header("location:SearchCategory.php?msg=Search Name cannot be blank");
}
